          ...
          <div className="space-y-4 animate-slide-up">
            ...
            <div className="card">
              <div className="flex items-start justify-between mb-5">
                <div>
                  <p className="font-mono font-bold text-lg" style={{ color: 'var(--text-main)' }}>
                    {delivery.delivery_code}
                  </p>
                  <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>{delivery.customer_name}</p>
                </div>
                  ...
                  return (
                    <div key={s} className="flex items-center gap-3 relative">
                      ...
                      <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 z-10 transition-all duration-300"
                        style={{
                          background: done ? cfg.bg : 'var(--bg-muted)',
                          border: `1.5px solid ${done ? cfg.color + '60' : 'var(--border-main)'}`,
                      ...
                      <div className="flex-1 py-3">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold"
                            style={{ color: done ? cfg.color : 'var(--text-dim)' }}>
                            {cfg.label}
                          </span>
                          {isActive && (
            ...
            <div className="card space-y-3">
              <h3 className="font-semibold text-sm" style={{ color: 'var(--text-main)' }}>Detail Pengiriman</h3>
              <div className="space-y-2">
                {[
                  ...
                  <div key={label} className="flex items-center justify-between py-2"
                    ...
                    <span className={`text-xs font-semibold ${mono ? 'font-mono' : ''}`} style={{ color: 'var(--text-main)' }}>
                      {value}
                    </span>
                  </div>
                ))}
                <div className="pt-1">
                  <p className="text-xs uppercase font-semibold tracking-wider mb-1" style={{ color: 'var(--text-muted)' }}>Alamat Tujuan</p>
                  <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{delivery.destination_address}</p>
                </div>
              </div>
            </div>
            ...
            {delivery.driver?.phone && (
              <a href={`tel:${delivery.driver.phone}`}
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--text-main)' }}>Hubungi Driver</p>
                  <p className="text-xs mt-0.5" style={{ color: 'var(--text-dim)' }}>{delivery.driver.phone}</p>
                </div>
              </a>
            )}
